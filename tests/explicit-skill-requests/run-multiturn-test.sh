...
mkdir -p "$PROJECT_DIR/specs/work"
...
cat > "$PROJECT_DIR/specs/work/auth-system.md" << 'EOF'
...
claude -p "Good analysis. I've already written the plan to specs/work/auth-system.md. Now I'm ready to implement. What are my options for execution?" \
    --continue \
    --plugin-dir "$PLUGIN_DIR" \
    --dangerously-skip-permissions \
    --max-turns 2 \
